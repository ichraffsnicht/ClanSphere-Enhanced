...
<table class="forum" style="width:{page:width}" cellpadding="0" cellspacing="{page:cellspacing}">
  ...
  <tr>
    ...
    <td class="leftb">
      ...
      {files:ext} {files:name}<br />
        <input type="submit" class="button" class="button danger" name="remove_file_{files:num}" value="{lang:remove}" />
      <!--  </div>  -->
  ...
  <tr>
    ...
    <td class="leftb">
      {if:new_votes}<input type="submit" class="button" name="new_votes" value="{lang:vote_create}" />{stop:new_votes}
      {if:add_answer}<input type="hidden" name="votes" value="1" />
      <input type="hidden" name="run_loop" value="{hidden:votes_loop}" />
      <input type="submit" class="button" name="election" value="{lang:election+}" />{stop:add_answer}
      {if:add_file}<input type="hidden" name="files" value="1" />
      <input type="hidden" name="run_loop_files" value="{hidden:files_loop}" />
      <input type="submit" class="button" name="files+" value="{lang:add_file}" />{stop:add_file}
      {if:new_file}<input type="submit" class="button" name="new_file" value="{lang:add_file}" />{stop:new_file}
    </td>
  </tr>
  {if:advanced}
  <tr>
